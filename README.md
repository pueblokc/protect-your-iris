...
- AMD GPUs work but with fewer hardware decode sessions
...
- **Blue Iris 6** — [blueirissoftware.com](https://blueirissoftware.com) ($70 one-time license)
- **UniFi Protect** — Running on a UniFi NVR (UNVR, UNVR Pro, UDM Pro, Cloud Key Gen2+)
- **UniFi Protect firmware** — 2.x or later (Integration API support)
...
- Enable "Direct-to-disc" for better I/O performance
...
1. Open the **UniFi Protect** web interface (or UniFi OS)
2. Go to **Settings** (gear icon)
3. Navigate to **System** → **Advanced**
4. Find **"Integration API"** and **enable** it
5. Click **"Generate API Key"**
6. **Save this key somewhere safe** — you'll need it for every API call
...
9. Click **"Test"** — you should see the video feed
10. Configure recording, motion, and other settings
...
RTSP tokens generated by the Integration API:
- Are generated fresh with each API call (new tokens each time)
- Persist across NVR reboots (unverified — may need regeneration)
- Can coexist — multiple active tokens for the same camera
- Don't have a known expiration time, but regenerate if cameras go NO_SIGNAL
